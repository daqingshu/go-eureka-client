...
func (t *EurekaServerApi) RegisterInstanceWithVo(vo *InstanceVo) (string, error) {
	var protocol = "http"
	if vo.SecurePort.Enabled {
		protocol = "https"
	}
	if vo.HomePageUrl == "" {
		vo.HomePageUrl = fmt.Sprintf("%s://%s:%d", protocol, vo.IpAddr, vo.Port.Value)
	}
	if vo.StatusPageUrl == "" {
	...
	if err != nil {
		log.Errorf("Failed to register app=%s, body=%v, err=%s", vo.App, string(body), err.Error())
		return "", err
	}
...
func (t *EurekaServerApi) GetApplication(appId string) (*ApplicationVo, error) {
	...
	var application = new(ApplicationVo)
	err = json.Unmarshal(res.Body(), &application)
	if err != nil {
